...
export default function WhitelistedEmailsPage() {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Delete this whitelisted email?')) return;
    setSaving(true);
    setError(null);
    const { error } = await supabase
      .from('whitelist_email_addresses')
  ...
  return (
    <div className="space-y-8 p-4">
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-bold uppercase tracking-[0.25em] mt-2">
            Manage individual e-mail addresses that are explicitly allowed
          </p>
        </div>
      ...
      <div className="bg-white rounded-[2rem] border border-dashed border-slate-300 p-6 space-y-4">
        ...
        >
          {saving ? 'Saving...' : 'Add Whitelisted Email'}
        </button>
      </div>
      ...
      <div className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="p-16 text-center text-xs font-black uppercase tracking-[0.35em] text-slate-300">
            No matching whitelisted e-mails found
          </div>
        ) : (
